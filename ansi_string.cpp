...
void AnsiString::collectionAsString( Collection<char> collection, char* rep )
{
    ...
    rep[ idx ] = NULL_CHAR ; // end of the string '\0'
...
#ifdef __DEBUG__
    assert( initialized ) ;
    assert( !( count + collection.size() > CONTAINER_CAPACITY ) ) ;
#endif // __DEBUG__
...
#ifdef __DEBUG__
    assert( intialized ) ;
    assert( !( count + string.length() > CONTAINER_CAPACITY ) ) ;
#endif // __DEBUG__
    
    Collection<char> result ;
    result.init( items, count ) ;
    result.add( string.items, string.size() ) ;
...
void AnsiString::dump( void ( *layout ) ( const char& ) ) const
{
